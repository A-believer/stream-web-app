...
function DownloadApp() {
  return (
    <section className='font-ppn text-foreground relative flex flex-col justify-center items-center font-medium xl:pt-[140px] xl:pb-[300px] text-center w-full xl:gap-y-[120px] md:gap-y-[100px] gap-y-[70px] mb-[100px]'>
      ...
      <div className="border w-fit mx-auto rounded-[15px] p-2.5 bg-[#138294] text-white md:rounded-[23.4px] px-6 py-4 border-[#138294] md:text-[28px] md:leading-[52x]my-5">Yes, download and join</div>
      <Image src={imgi} alt='Image I' className='absolute xl:block hidden left-10 top-60'/>
      <Image src={imgii} alt='Image II' className='absolute xl:block hidden right-10 top-60'/>
      <Image src={imgiii} alt='Image III' className='absolute xl:block hidden left-10 bottom-0'/>
      <Image src={imgiv} alt='Image IV' className='absolute xl:block hidden right-10 bottom-0'/>
    </section>
  )
}
